...
Prerequisites are Docker and Git for both scenario's. For scenario 2 you also need Python 3.11 and a recent version of Node.js (we test with the Long Term Support version of Node).
...
*Quality-time* has one component that only contain shared code. The shared code is used by all Python components.
...
1. Add a specification of the new metric to the data model. See the documentation of the [shared code](software.md#shared-code) component for a description of the data model and the different metric fields.
...
Be sure to run the unit tests of the shared code after adding a metric to the data model, to check the integrity of the data model. If you forget to do step 2 above, one of the tests will fail. Other than changing the data model, no code changes are needed to support new metrics.
...
```console
cd components/external_server  # or components/collector, components/notifier, components/shared_code
ci/unittest.sh
```
...
```console
docker-compose up -d
docker run -it -w `pwd` -v `pwd`:`pwd` --network=container:qualitytime_www_1 python:3.11.4-bullseye tests/application_tests/ci/test.sh
```
