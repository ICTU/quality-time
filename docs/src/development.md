...
*Quality-time* has two components that only contain shared code. The shared data model is used by all Python components. The code in the shared code component is used by the external server.
...
```console
cd components/external_server  # or components/shared_data_model, components/shared_code, components/collector, components/notifier
ci/unittest.sh
```
