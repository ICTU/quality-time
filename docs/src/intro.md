...
- *Quality-time* provides a single overview of the software measurements, but refers users to the underlying sources for detailed information.
...
- *Quality-time* reports on the (un)availability of sources and can measure information about sources, such as the version of a source or the up-to-dateness of information, but the correct configuration of sources, for example whether certain rules are activated, is out of scope for *Quality-time*.
