...
function add_notification_destination(report_uuid, reload) {
    return fetch_server_api('post', `report/${report_uuid}/destinations/new`, {}).then(reload)
}

function delete_notification_destination(report_uuid, destination_uuid, go_home) {
    return fetch_server_api('delete', `report/${report_uuid}/destinations/${destination_uuid}`, {}).then(go_home)
}

function set_notification_destination_attribute(report_uuid, destination_uuid, attribute, value, reload) {
    return fetch_server_api('post', `report/${report_uuid}/destinations/${destination_uuid}/attribute/${attribute}`, { [attribute]: value }).then(reload)
}
