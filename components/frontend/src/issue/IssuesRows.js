...
function CreateIssueButton({
    ...
    return (
        <ActionButton
            ...
            popup={
                <>
                    Create a new issue for this {target} in the configured issue tracker and add its
                    identifier to the tracked issue identifiers.{issueTrackerInstruction}
                </>
            }
            position="top center"
        />
...
function IssueIdentifiers({
    ...
    const issueStatusHelp = (
        <>
            <p>
                Identifiers of issues in the configured issue tracker that track the progress of
                fixing this {target}.
            </p>
            <p>
                When the issues have all been resolved, or the technical debt end date has passed,
                whichever happens first, the technical debt should be resolved and the technical
                debt target is no longer evaluated.
            </p>
            {issueTrackerInstruction}
        </>
    )
    ...
    return (
        <MultipleChoiceInput
            ...
            onSearchChange={(query) => {
                if (query) {
                    get_report_issue_tracker_suggestions(report_uuid, query).then(
                        (suggestionsResponse) => {
            ...
            label={
                <LabelWithHelp labelId={labelId} label="Issue identifiers" help={issueStatusHelp} />
            }
            options={suggestions}
...
export function IssuesRows({ metric, metric_uuid, reload, report, target }) {
    ...
    const issueTrackerInstruction = issueTrackerConfigured ? null : (
        <p>
            Please configure an issue tracker by expanding the report title, selecting the
            &apos;Issue tracker&apos; tab, and configuring an issue tracker.
        </p>
    )
        ...
        <>
            ...
            {getMetricIssueIds(metric).length > 0 && !issueTrackerConfigured && (
                <Grid.Row>
                    <Grid.Column width={16}>
                        <ErrorMessage
                            title="No issue tracker configured"
                            message={issueTrackerInstruction}
                        />
                    </Grid.Column>
                </Grid.Row>
            )}
