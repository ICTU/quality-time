...
it('logs in', async () => {
    ...
    await act(async () => { render(<Menubar report_date_string="2019-10-10" onDate={() => {/* Dummy handler */ }} user={null} set_user={set_user} />); });
...
it('logs out', async () => {
    ...
    const set_user = jest.fn();
    await act(async () => { render(<Menubar user={"jadoe"} set_user={set_user} />); });
...
it('does not go to home page if on reports overview', async () => {
    ...
    await act(async () => { render(<Menubar atHome={true} go_home={go_home} />); });
...
it('goes to home page on keypress', async () => {
    const go_home = jest.fn();
    await act(async () => { render(<Menubar go_home={go_home} />); });
...
["{Enter}", " ", "x"].forEach(key => {
    it('shows the view panel on enter', async () => {
        await act(async () => { render(<Menubar atHome={true} panel={<div>Hello</div>} />); });
...
it("clears the visible details tabs", async () => {
    ...
    await act(async () => { render(<Menubar clearVisibleDetailsTabs={clearVisibleDetailsTabs} visibleDetailsTabs={["tab"]} />) });
...
it("doesn't clear the visible details tabs if there are none", async () => {
    const clearVisibleDetailsTabs = jest.fn();
    await act(async () => { render(<Menubar clearVisibleDetailsTabs={clearVisibleDetailsTabs} visibleDetailsTabs={[]} />) });
    fireEvent.click(screen.getByRole("button", { name: "Collapse all metrics" }))
