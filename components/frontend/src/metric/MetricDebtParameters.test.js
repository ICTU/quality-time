...
it('unaccepts technical debt and resets target and end date', async () => {
    fetch_server_api.fetch_server_api = jest.fn().mockResolvedValue({ ok: true });
    await act(async () => { renderMetricDebtParameters({ accept_debt: true }) });
...
it('does not show an error message if the metric has no issues and an issue tracker is configured', async () => {
    await act(async () => { renderMetricDebtParameters({ report: { issue_tracker: { type: "Jira" } } }) });
    expect(screen.queryAllByText(/No issue tracker configured/).length).toBe(0);
});

it('does not show an error message if the metric has issues and an issue tracker is configured', async () => {
    await act(async () => { renderMetricDebtParameters({ issue_ids: ["BAR-42"], report: { issue_tracker: { type: "Jira", parameters: { url: "https://jira", project_key: "KEY", issue_type: "Bug" } } } }) });
    expect(screen.queryAllByText(/No issue tracker configured/).length).toBe(0);
});

it('shows an error message if the metric has issues but no issue tracker is configured', async () => {
    await act(async () => { renderMetricDebtParameters({ issue_ids: ["FOO-42"] }) });
    expect(screen.queryAllByText(/No issue tracker configured/).length).toBe(1);
});

it('shows a connection error', async () => {
    await act(async () => { renderMetricDebtParameters({ issue_status: [{ issue_id: "FOO-43", connection_error: "Oops" }] }) });
...
it('shows a parse error', async () => {
    await act(async () => { renderMetricDebtParameters({ issue_status: [{ issue_id: "FOO-43", parse_error: "Oops" }] }) });
...
it('creates an issue', async () => {
    ...
    await act(async () => { renderMetricDebtParameters({ report: reportWithIssueTracker }) });
...
it('does not show the create issue button if the user has no permissions', async () => {
    await act(async () => { renderMetricDebtParameters({ report: reportWithIssueTracker, permissions: [] }) });
...
it('shows issue id suggestions', async () => {
    fetch_server_api.fetch_server_api = jest.fn().mockResolvedValue({ suggestions: [{ key: "FOO-42", text: "Suggestion" }] });
    await act(async () => { renderMetricDebtParameters({ report: { issue_tracker: { type: "Jira", parameters: { url: "https://jira" } } } }) });
