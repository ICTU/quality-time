...
it('shows help for evaluated metric with tech debt', async () => {
    renderMetricTarget({type: "violations", target: "10", debt_target: "15", near_target: "20", accept_debt: true});
    await userEvent.tab()
    await waitFor(() => {
...
it('shows help for evaluated metric with tech debt with end date', async () => {
    renderMetricTarget({type: "violations", target: "10", debt_target: "15", near_target: "20", accept_debt: true, debt_end_date: "3000-01-01"});
    await userEvent.tab()
    await waitFor(() => {
        assertVisible(/Target met/, /≦ 10 violations/, /Debt target met/, /10 - 15 violations/, /Near target met/, /15 - 20 violations/, /Target not met/, /> 20 violations/)
    });
...
it('shows help for evaluated metric with tech debt with end date in the past', async () => {
    renderMetricTarget({type: "violations", target: "10", debt_target: "15", near_target: "20", accept_debt: true, debt_end_date: "2000-01-01"});
    await userEvent.tab()
    await waitFor(() => {
        assertVisible(/Target met/, /≦ 10 violations/, /Near target met/, /10 - 20 violations/, /Target not met/, /> 20 violations/)
        assertNotVisible(/Debt target met/)
...
it('shows help for evaluated metric with tech debt completely overlapping near target', async () => {
    renderMetricTarget({type: "violations", target: "10", debt_target: "20", near_target: "20", accept_debt: true});
    await userEvent.tab()
    await waitFor(() => {
...
it('shows help for evaluated more-is-better metric with tech debt', async () => {
    renderMetricTarget({type: "violations", target: "15", near_target: "5", debt_target: "10", accept_debt: true, direction: ">"});
    await userEvent.tab()
    await waitFor(() => {
...
it('shows help for evaluated more-is-better metric with tech debt and missing debt target', async () => {
    renderMetricTarget({type: "violations", target: "15", near_target: "5", accept_debt: true, direction: ">"});
    await userEvent.tab()
    await waitFor(() => {
...
it('shows help for evaluated more-is-better metric with tech debt completely overlapping near target', async () => {
    renderMetricTarget({type: "violations", target: "15", near_target: "5", debt_target: "5", accept_debt: true, direction: ">"});
    await userEvent.tab()
    await waitFor(() => {
