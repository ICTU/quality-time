...
export function DivWithHtml({ children }) {
    ...
    useLayoutEffect(() => {
        setOverflown(ref.current.clientHeight < ref.current.scrollHeight) // eslint-disable-line @eslint-react/hooks-extra/no-direct-set-state-in-use-effect -- DOM measurement must happen after layout
    }, [children])
    return (
        <div
            className="with-html"
