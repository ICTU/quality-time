...
JiraFieldOptionsJSON = dict[str, list[dict[str, str]]]
...
JiraIssuesJSON = dict[str, list[dict[str, str | dict[str, str]]]]
JiraJSON = JiraProjectsJSON | JiraFieldOptionsJSON | JiraIssueTypesJSON | JiraIssueSuggestionJSON | JiraIssuesJSON
...
class IssueTracker:
    ...
    def __get_field_options(self, issue_types: list[Option]) -> list[Option]:  # pragma: no feature-test-cover
        ...
        try:
            fields = cast(JiraFieldOptionsJSON, self.__get_json(api_url))["values"]
        except Exception as reason:
    ...
    def __get_epic_links(self, fields: list[Option]) -> list[Option]:  # pragma: no feature-test-cover
        ...
        epics = []
        try:
            epics = cast(JiraIssuesJSON, self.__get_json(api_url))["issues"]
        except Exception as reason:
            logging.warning("Getting epics at %s failed: %s", api_url, reason)
        return [
            Option(
                str(epic["key"]),
                str(f'{cast(dict[str, dict[str, str]], epic)["fields"]["summary"]} ({epic["key"]})'),
            )
            for epic in epics
        ]
