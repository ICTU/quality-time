...
class PostSubjectAttributeTest(DataModelTestCase):
    ...
    def setUp(self):
        ...
        self.report = Report(
            self.DATA_MODEL,
            {
                ...
                "title": "Report",
                "subjects": {SUBJECT_ID: {"name": "subject1"}, SUBJECT_ID2: {"type": "software"}},
            },
        )
        self.database.reports.find.return_value = [self.report]
        self.database.measurements.find.return_value = []
        self.email = "john@example.org"
        self.database.sessions.find_one.return_value = {"user": "John", "email": self.email}

    def assert_delta(self, delta: str, subject_id: SubjectId, report: dict) -> None:
        """Check that the delta is correct."""
        self.assertEqual(
            {"uuids": [REPORT_ID, subject_id], "email": self.email, "description": f"John changed the {delta}."},
            report["delta"],
        )
    ...
    def test_post_position_first(self, request):
        ...
        self.assert_delta(
            "position of subject 'Software' in report 'Report' from '1' to '0'",
            SUBJECT_ID2,
            updated_report,
        )
    ...
    def test_post_position_last(self, request):
        ...
        self.assertEqual([SUBJECT_ID2, SUBJECT_ID], list(updated_report["subjects"].keys()))
        self.assert_delta(
            "position of subject 'subject1' in report 'Report' from '0' to '1'",
            SUBJECT_ID,
            updated_report,
        )
...
class SubjectTest(DataModelTestCase):
    ...
    def setUp(self):
        """Extend to set up database contents."""
        super().setUp()
    ...
    def assert_delta(self, delta: str, uuids, report=None) -> None:
        ...
        self.assertEqual(
            {"uuids": sorted(uuids), "email": self.email, "description": f"Jenny {delta}."},
            report["delta"],
        )
    ...
    def test_add_subject(self, request):
        ...
        self.assertEqual("software", updated_report["subjects"][subject_uuid]["type"])
    ...
    def test_delete_subject(self):
        ...
        self.assertEqual({"ok": True}, delete_subject(SUBJECT_ID, self.database))
        updated_report = self.database.reports.insert_one.call_args[0][0]
    ...
    def test_move_subject(self):
        ...
        self.assertEqual({"ok": True}, post_move_subject(SUBJECT_ID, REPORT_ID2, self.database))
