"""Unit tests for shared measurement functions."""
...
class DetermineMeasurementStatusTest(unittest.TestCase):
    ...
    def setUp(self):
        self.datamodel = dict(_id="", metrics=dict(metric_type=dict(direction="≦")))

    def test_green(self):
        """Test a green measurement."""
        metric = dict(type="metric_type", target="20", near_target="15", debt_target=None, accept_debt=False)
        self.assertEqual(
            "target_met", determine_measurement_status(self.datamodel, metric, "10"))

    def test_yellow(self):
        """Test a yellow measurement."""
        metric = dict(type="metric_type", target="20", near_target="25", debt_target=None, accept_debt=False)
        self.assertEqual(
            "near_target_met", determine_measurement_status(self.datamodel, metric, "22"))

    def test_red(self):
        """Test a red measurement."""
        metric = dict(type="metric_type", target="20", near_target="25", debt_target=None, accept_debt=False)
        self.assertEqual(
            "target_not_met", determine_measurement_status(self.datamodel, metric, "30"))
    ...
    def test_debt_met(self):
        ...
        self.assertEqual(
            "debt_target_met", determine_measurement_status(self.datamodel, metric, "30"))

    def test_debt_not_met(self):
        """Test a measurement worse than the accepted debt."""
        metric = dict(type="metric_type", target="20", near_target="25", debt_target="30", accept_debt=True)
        self.assertEqual(
            "target_not_met", determine_measurement_status(self.datamodel, metric, "35"))
    ...
    def test_debt_past_end_date(self):
        ...
        self.assertEqual(
            "target_not_met", determine_measurement_status(self.datamodel, metric, "29"))

    def test_green_with_debt(self):
        """Test a measurement with debt, better than the target."""
        metric = dict(type="metric_type", target="20", near_target="25", debt_target="30", accept_debt=True)
        self.assertEqual(
            "target_met", determine_measurement_status(self.datamodel, metric, "15"))
