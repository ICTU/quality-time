...
def get_database(client: pymongo.MongoClient = get_client()) -> Database:
    """ Get the Quality Time database connection.

    Returns:
        Database: pointer to the Quality Time database.
    """
...
def init_database() -> Database:  # pragma: no feature-test-cover
    """Initialize the database, create indexes, import the datamodel and import example reports.
    ...
    logging.info("Database has %d report documents and %d measurement documents",
                 nr_reports, nr_measurements)
...
def create_indexes(database: Database) -> None:
    ...
    latest_measurement_index = pymongo.IndexModel(
        [("metric_uuid", pymongo.ASCENDING), ("start", pymongo.DESCENDING)])
    latest_successful_measurement_index = pymongo.IndexModel(
        [("metric_uuid", pymongo.ASCENDING), ("has_error",
                                              pymongo.ASCENDING), ("start", pymongo.DESCENDING)]
    )
    database.measurements.create_indexes(
        [start_index, latest_measurement_index, latest_successful_measurement_index])
