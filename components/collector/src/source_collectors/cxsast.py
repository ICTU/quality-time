...
class CxSASTBase(SourceCollector, ABC):  # pylint: disable=abstract-method
    ...
    async def _get_source_responses(self, session: aiohttp.ClientSession, api_url: URL) -> Responses:
        ...
        token_response = await self.__api_post(session, "auth/identity/connect/token", credentials)
    ...
    async def __api_post(self, session: aiohttp.ClientSession, api: str, data) -> aiohttp.ClientResponse:
        """Post to the API and return the response."""
        timeout = aiohttp.ClientTimeout(self.TIMEOUT)
        response = await session.post(f"{self._api_url()}/cxrestapi/{api}", data=data, timeout=timeout)
...
class CxSASTSecurityWarnings(CxSASTBase):
    ...
    async def _get_source_responses(self, session: aiohttp.ClientSession, api_url: URL) -> Responses:
        await super()._get_source_responses(session, api_url)
