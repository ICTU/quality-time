...
class DependencyTrackLatestVersionStatusBase(DependencyTrackBase):
    ...
    def _parent_component_attributes(self, uuid: str, graph: DependencyTrackComponentGraph) -> dict[str, str]:
        """Return the parent component entity attributes for the component with the specified UUID."""
        roots = graph.root_components(uuid)
        attributes = {"parent_component": ", ".join(roots.values())}
        if len(roots) == 1:  # Entity attributes support at most one URL, so only link if there's exactly one parent
            attributes["parent_component_landing_url"] = self._landing_url_of_component(next(iter(roots)))
        return attributes
