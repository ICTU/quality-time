...
class AxeHTMLReporterAccessibility(HTMLFileSourceCollector, AxeAccessibilityCollector):
    ...
    def __parse_html_soup(self, soup: Tag) -> Iterator[dict[str, str]]:
        ...
        for result_type in self._parameter("result_types"):
            if result_type == "violations":
                ...
                yield from self.__parse_rules_from_html_soup(
                    soup, page_url, result_type
                )

    def __parse_violations_from_html_soup(
        self, soup: Tag, page_url: str
        ...
        for violated_rule in soup.select("div.violationCard > div.card-body"):
            ...
            description = cast(Tag, violated_rule.select_one("p.card-text")).get_text(
                strip=True
            )
            help_url = cast(
                str, cast(Tag, violated_rule.select_one("a.learnMore"))["href"]
            )
    ...
    @staticmethod
    def __parse_rules_from_html_soup(
        soup: Tag, page_url: str, result_type: str
    ) -> Iterator[dict[str, str]]:
    ...
    def __parse_tags(violated_rule: Tag) -> list[str]:
        ...
        tags_list = [
            node for node in violated_rule("h6") if "Issue Tags:" in node.get_text()
        ]
    ...
    def __parse_element(violation: Tag) -> str:
        ...
        element_tags = violation("td")[1].find_all(
