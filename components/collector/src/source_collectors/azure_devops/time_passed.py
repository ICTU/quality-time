"""Azure Devops Server time passed collector."""
...
class AzureDevopsFileTimePassed(TimePassedCollector, AzureDevopsRepositoryBase):
    """Collector class to measure the time passed since a repo or folder/file in a repo was last changed."""
...
class AzureDevopsJobTimePassed(TimePassedCollector, AzureDevopsJobs):  # lgtm [py/conflicting-attributes]
    """Collector class to measure the time passed since a job/pipeline ran."""
...
class AzureDevopsTimePassed(SourceCollector, ABC):
    ...
    def __new__(cls, session: aiohttp.ClientSession, source, data_model):
        ...
        collector_class = AzureDevopsFileTimePassed if file_path else AzureDevopsJobTimePassed
