...
class HarborSecurityWarnings(HarborBase):
    ...
    async def _get_source_responses(self, *urls: URL, **kwargs) -> SourceResponses:
        ...
        projects_responses = await super()._get_source_responses(projects_api, **kwargs)
        for projects_response in projects_responses:
            for project in await projects_response.json():
                ...
                responses.extend(await self._get_source_responses_for_project(project, repositories_api, **kwargs))
        return responses

    async def _get_source_responses_for_project(self, project, repositories_api: URL, **kwargs) -> SourceResponses:
        """Return the source responses for a specific project."""
        responses = SourceResponses()
        repositories_responses = await super()._get_source_responses(repositories_api, **kwargs)
        for repositories_response in repositories_responses:
            for repository in await repositories_response.json():
                ...
                responses.extend(await super()._get_source_responses(artifacts_api, **kwargs))
        return responses
