...
class OWASPDependencyCheckJenkinsPluginSecurityWarnings(SourceCollector):
    ...
    def api_url(self) -> URL:
        return URL(f"{super().api_url()}/lastSuccessfulBuild/dependency-check-jenkins-pluginResult/api/json?depth=1")

    def landing_url(self, responses: List[requests.Response]) -> URL:
        return URL(f"{super().api_url()}/lastSuccessfulBuild/dependency-check-jenkins-pluginResult")
        ...
        for warning in warnings:
            ...
            if file_path in entities:
                ...
                entities[file_path]["highest_severity"] = \
                    self.highest_severity(str(entities[file_path]["highest_severity"]).lower(), priority).capitalize()
            else:
                entities[file_path] = dict(
                    key=file_path, file_path=file_path, highest_severity=priority.capitalize(), nr_vulnerabilities=1)
        return list(entities.values())

    def highest_severity(self, severity1: str, severity2: str) -> str:
...
class OWASPDependencyCheckJenkinsPluginSourceUpToDateness(SourceCollector):
    ...
    def api_url(self) -> URL:
        return URL(f"{super().api_url()}/lastSuccessfulBuild/api/json")
