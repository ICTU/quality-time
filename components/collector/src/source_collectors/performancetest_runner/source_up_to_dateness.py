...
class PerformanceTestRunnerSourceUpToDateness(
    PerformanceTestRunnerBaseClass, TimePassedCollector
):
    ...
    async def _parse_source_response_date_time(self, response: Response) -> datetime:
        ...
        datetime_parts = [
            int(part) for part in (await self.__start_of_the_test(response)).split(".")
        ]
