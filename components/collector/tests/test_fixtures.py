...
class FixtureTest(unittest.TestCase):
    ...
    def test_create_report(self):
        ...
        assert report1["report_uuid"] == "report1"
        assert report1["title"] == "Title"
        assert report1["subjects"][SUBJECT_ID]["metrics"][METRIC_ID]["name"] == "Metric"

    def test_create_report_deleted(self):
        """Tests with deleted"""
        report_deleted = create_report(deleted=True)
        assert report_deleted["deleted"]

    def test_create_report_with_title(self):
        """Tests with title"""
        report_with_alternate_title = create_report(title="Foo")
        assert report_with_alternate_title["title"] == "Foo"

    def test_create_report_with_kwargs(self):
        """Tests with kwargs"""
        report_with_kwargs = create_report(last=True, metric_id=METRIC_ID2)
        assert report_with_kwargs["last"]
        assert report_with_kwargs["subjects"][SUBJECT_ID]["metrics"][METRIC_ID2] is not None
    ...
    def test_create_report_with_other_kwargs(self):
        ...
        assert report_with_kwargs["subjects"][SUBJECT_ID]["metrics"][METRIC_ID]["type"] == "foo"
