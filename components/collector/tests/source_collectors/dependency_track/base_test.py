...
class DependencyTrackTestCase(SourceCollectorTestCase):
    ...
    def parent_component_attributes(self, name: str, uuid: str = "") -> dict[str, str]:
        """Create the expected parent component entity attributes. Pass the UUID if a landing URL is expected."""
        attributes = {"parent_component": name}
        if uuid:
            attributes["parent_component_landing_url"] = f"{self.landing_url}/components/{uuid}"
        return attributes
