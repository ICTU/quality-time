...
class JMeterJSONTestsTest(JMeterJSONTestCase):
    ...
    async def test_all_samples(self):
        ...
        measurement = await self.collect_measurement(get_request_json_return_value=self.JMETER_JSON)
        self.assert_measurement(measurement, value="248")
    ...
    async def test_successful_samples(self):
        ...
        measurement = await self.collect_measurement(get_request_json_return_value=self.JMETER_JSON)
        self.assert_measurement(measurement, value="242")
    ...
    async def test_ignore_transaction(self):
        ...
        measurement = await self.collect_measurement(get_request_json_return_value=self.JMETER_JSON)
        self.assert_measurement(measurement, value="123")
