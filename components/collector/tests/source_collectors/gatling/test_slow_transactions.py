...
class GatlingSlowTransactionsTest(GatlingTestCase):
    ...
    async def test_all_transactions(self):
        """Test retrieving all slow transaction."""
    ...
    async def test_api_url_ending_with_index_html(self):
        """Test that an API URL that ends with index.html is not changed."""
        self.set_source_parameter("url", "https://gatling/index.html")
        response = await self.collect(get_request_text="")
        self.assert_measurement(response, api_url="https://gatling/index.html")

    async def test_api_url_not_ending_with_index_html(self):
        """Test that an API URL that does not end with index.html get index.html added."""
        self.set_source_parameter("url", "https://gatling/report/")
        response = await self.collect(get_request_text="")
        self.assert_measurement(response, api_url="https://gatling/report/index.html")
