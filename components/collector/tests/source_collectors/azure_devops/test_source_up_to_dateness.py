...
class AzureDevopsSourceUpToDatenessTest(AzureDevopsTestCase):
    ...
    async def test_age_of_file(self):
        ...
        measurement = await self.collect_measurement(get_request_json_side_effect=[repositories, commits])
    ...
    async def test_age_of_pipeline(self):
        ...
        measurement = await self.collect_measurement(get_request_json_return_value=self.build_json)
        self.assert_measurement(measurement, value=self.expected_age, landing_url=f"{self.url}/_build")
