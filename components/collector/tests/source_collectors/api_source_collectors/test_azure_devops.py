...
class AzureDevopsTestCase(SourceCollectorTestCase):
    ...
    def setUp(self):
        ...
        self.sources = dict(
            source_id=dict(type="azure_devops", parameters=dict(url=self.url, private_token="xxx")))
        self.work_item = dict(
            id="id", url=self.work_item_url,
...
class AzureDevopsIssuesTest(AzureDevopsTestCase):
    ...
    async def test_issues(self):
        ...
        self.assert_measurement(
            response,
            entities=[dict(
                key="id", project="Project", title="Title", work_item_type="Task", state="New",
...
class AzureDevopsTestsTest(AzureDevopsTestCase):
    ...
    async def test_nr_of_tests(self):
        ...
        self.assert_measurement(response, value="9", total="10", entities=[
            dict(key="2", name="A", state="Completed", build_id="2", started_date="2020-06-20T14:56:00.58Z",
                 completed_date="2020-06-20T14:56:00.633Z", counted_tests="2", incomplete_tests="0",
...
class AzureDevopsFailedJobsTest(AzureDevopsTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.path = r"\\folder"
        self.pipeline = r"folder/pipeline"
    ...
    async def test_nr_of_failed_jobs(self):
        ...
        self.sources["source_id"]["parameters"]["jobs_to_ignore"] = ["ignore_by_name", "folder/ignore.*"]
        metric = dict(type="failed_jobs", sources=self.sources, addition="sum")
        response = await self.collect(
            metric,
            get_request_json_return_value=dict(
                value=[
                    dict(path=self.path, name="pipeline", _links=dict(web=dict(href=f"{self.url}/build")),
                         latestCompletedBuild=dict(result="failed", finishTime="2019-11-15T12:24:10.1905868Z")),
                    dict(path=fr"{self.path}\\subfolder", name="pipeline",
