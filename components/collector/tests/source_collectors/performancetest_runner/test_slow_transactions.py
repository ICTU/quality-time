...
class PerformanceTestRunnerSlowTransactionsTest(PerformanceTestRunnerTestCase):
    ...
    async def test_warning_only(self):
        ...
        self.assert_measurement(
            measurement, value="1", entities=[{"key": "Name", "name": "Name", "threshold": "warning"}]
        )
    ...
    async def test_ignore_transactions_by_name(self):
        ...
        measurement = await self.collect_measurement(get_request_text=html)
        self.assert_measurement(measurement, value="1", entities=[{"key": "T2", "name": "T2", "threshold": "warning"}])
