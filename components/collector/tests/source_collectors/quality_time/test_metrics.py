...
class QualityTimeMetricsTest(QualityTimeTestCase):
    ...
    async def test_nr_of_metrics(self):
        ...
        measurement = await self.collect_measurement(get_request_json_return_value=self.reports)
        ...
        self.assert_measurement(measurement, value="1", total="3", entities=self.entities)

    async def test_nr_of_metrics_without_reports(self):
        """Test that an error is thrown if no reports exist."""
        measurement = await self.collect_measurement(get_request_json_return_value={"reports": []})
        self.assert_measurement(measurement, parse_error="No reports found")
