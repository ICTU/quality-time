...
class JenkinsFailedJobsTest(JenkinsTestCase):
    ...
    async def test_failed_child_job(self):
        ...
        self.assert_measurement(measurement, value="3")

    async def test_failed_jobs(self):
        """Test that the failed jobs are returned."""
        measurement = await self.collect_measurement(get_request_json_return_value=self.jenkins_json(nr_jobs=1))
        self.assert_measurement(measurement, entities=[self.expected_entity()])
    ...
    async def test_ignore_unbuildable_failed_jobs(self):
        ...
        measurement = await self.collect_measurement(
            get_request_json_return_value=self.jenkins_json(nr_jobs=1, buildable=False)
        )
        self.assert_measurement(measurement, entities=[])
    ...
    async def test_include_jobs(self):
        ...
        measurement = await self.collect_measurement(get_request_json_return_value=self.jenkins_json(nr_jobs=2))
        self.assert_measurement(measurement, entities=[self.expected_entity()])
    ...
    async def test_include_jobs_by_regular_expression(self):
        ...
        measurement = await self.collect_measurement(get_request_json_return_value=self.jenkins_json(nr_jobs=2))
        self.assert_measurement(measurement, entities=[self.expected_entity(job_nr=2)])
    ...
    async def test_no_builds(self):
        """Test no builds."""
        measurement = await self.collect_measurement(
            get_request_json_return_value=self.jenkins_json(nr_jobs=1, builds=[])
        )
        self.assert_measurement(measurement, entities=[])
    ...
    async def test_ignore_job_if_all_failed_builds_are_in_the_grace_period(self):
        ...
        measurement = await self.collect_measurement(
            get_request_json_return_value=self.jenkins_json(nr_jobs=1, builds=builds)
        )
        self.assert_measurement(measurement, entities=[])
    ...
    async def test_do_not_ignore_job_if_not_all_failed_builds_are_in_the_grace_period(self):
        ...
        self.assert_measurement(measurement, entities=[expected_entity])
