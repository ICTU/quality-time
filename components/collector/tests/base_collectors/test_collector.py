...
class CollectorTest(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self.metrics = {
            "metric_uuid": {
                ...
                "addition": "sum",
                "type": "dependencies",
                "sources": {"source_id": {"type": "pip", "parameters": {"url": self.url}}},
            },
        }
        self.pip_json = [{"name": "a dependency"}]

    @staticmethod
    def _patched_get(mock_async_get_request: AsyncMock, side_effect=None) -> Mock:
    ...
    @staticmethod
    def _patched_post(side_effect=None) -> Mock:
    ...
    async def _fetch_measurements(self, mock_async_get_request: AsyncMock, number: int = 1, side_effect=None) -> None:
    ...
    async def test_fetch_with_unsupported_collector(self):
        ...
        self.metrics["metric_uuid"]["sources"]["source_id"] = {"type": "unsupported_source"}
        mock_async_get_request = AsyncMock()
        mock_async_get_request.json.return_value = self.metrics
        with self._patched_post() as post:
            await self._fetch_measurements(mock_async_get_request)
    ...
    async def test_fetch_with_client_error(self):
        ...
        with self._patched_post() as post:
            await self._fetch_measurements(
                None,
                side_effect=[mock_async_get_request, aiohttp.ClientConnectionError("error")],
            )
        post.assert_called_once_with(
            self.measurement_api_url,
            json={
