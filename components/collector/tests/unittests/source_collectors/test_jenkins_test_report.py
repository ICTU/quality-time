...
class JenkinsTestReportTest(unittest.TestCase):
    ...
    def setUp(self):
        self.datamodel = dict(
            sources=dict(jenkins_test_report=dict(parameters=dict(failure_type=dict(values=["failed", "skipped"])))))
        self.mock_response = Mock()

    def test_nr_of_tests(self):
        """Test that the number of tests is returned."""
        self.mock_response.json = Mock(return_value=dict(passCount=4, failCount=2))
        metric = dict(
    ...
    def test_nr_of_failed_tests(self):
        ...
        self.mock_response.json = Mock(return_value=dict(
    ...
    def test_source_up_to_dateness(self):
        ...
        self.mock_response.json = Mock(return_value=dict(suites=[dict(timestamp="2019-04-02T08:52:50")]))
        metric = dict(
