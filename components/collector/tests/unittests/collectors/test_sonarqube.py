...
class SonarQubeTest(unittest.TestCase):
    ...
    def test_version(self):
        ...
        metric = dict(metric="version", sources=dict(a=dict(source="sonarqube", url="http://sonar")))
    ...
    def test_violations(self):
        ...
        metric = dict(
            metric="violations", sources=dict(a=dict(source="sonarqube", url="http://sonar", component="id")))
        with patch("requests.get", return_value=mock_response):
            response = Collector(metric).get()
    ...
    def test_tests(self):
        ...
        metric = dict(
            metric="tests", sources=dict(a=dict(source="sonarqube", url="http://sonar", component="id")))
        with patch("requests.get", return_value=mock_response):
            response = Collector(metric).get()
    ...
    def test_covered_lines(self):
        ...
        metric = dict(
            metric="covered_lines", sources=dict(a=dict(source="sonarqube", url="http://sonar", component="id")))
        with patch("requests.get", return_value=mock_response):
            response = Collector(metric).get()
    ...
    def test_covered_branches(self):
        ...
        metric = dict(
            metric="covered_branches", sources=dict(a=dict(source="sonarqube", url="http://sonar", component="id")))
        with patch("requests.get", return_value=mock_response):
            response = Collector(metric).get()
        self.assertEqual("90", response["sources"][0]["measurement"])
