...
class SonarQubeTest(unittest.TestCase):
    ...
    def test_violations(self):
        ...
        mock_response.json.return_value = dict(
            total="2",
            issues=[
                dict(key="a", message="a", component="a", severity="INFO"),
                dict(key="b", message="b", component="b", severity="MAJOR")])
        sources = dict(
        ...
        self.assertEqual(
            [
                dict(component="a", key="a", message="a", severity="info",
                     url="http://sonar/project/issues?id=id&open=a"),
                dict(component="b", key="b", message="b", severity="major",
