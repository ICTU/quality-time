...
class SonarQubeTest(unittest.TestCase):
    ...
    def test_violations(self):
        ...
        sources = dict(
            a=dict(type="sonarqube", url="http://sonar", component="id"))
        with patch("requests.get", return_value=mock_response):
            response = Collector().get("violations", sources)
        self.assertEqual([dict(component="a", key="a", message="a", url="None/project/issues?id=None&open=a"),
                          dict(component="b", key="b", message="b", url="None/project/issues?id=None&open=b")],
                         response["sources"][0]["measurement"])
