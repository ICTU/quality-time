...
Given(/^I make sure the Metric debt target is (.+), Metric target is (.+) and Accept technical debt (.+)$/, (debt, metric, technicalDebt) => {
    cy.get("#root > div.ui.fluid.container > div.ui.doubling.stackable.five.cards > a:nth-child(1)").click();

    cy.get("#root > div.ui.fluid.container > div.ui.basic.segment > table > tbody > tr:nth-child(3)").within(() => {
        cy.get('td.collapsing').click();
    });

    cy.get("#root > div.ui.fluid.container > div.ui.basic.segment > table > tbody > tr:nth-child(4)").within(() => {
