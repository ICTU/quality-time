...
Given(/^I make sure the Metric debt target is (.+), Metric target is (.+) and Accept technical debt (.+)$/, (debt, metric, technicalDebt) => {
    cy.get("#root > div.ui.fluid.container >  div:nth-child(4) > table > tbody > tr:nth-child(3)").within(() => {
        cy.get('td.collapsing > i.caret.right').click();
    });

    cy.get("#root > div.ui.fluid.container >  div:nth-child(4) > table > tbody > tr:nth-child(4)").within(() => {
