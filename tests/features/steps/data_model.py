...
def get_data_model(context):
    ...
    headers = {"If-None-Match": f"W/{md5_hash(context.response['timestamp'])}"} if context.response else {}
    context.response = context.get("datamodel", headers=headers)


@when("the client gets a data model from too long ago")
def get_old_data_model(context):
    """Get a data model from too long ago."""
    context.response = context.get(f"datamodel?report_date=2000-07-24T10:59:58+00:00")


@then("the server returns the most recent data model")
def check_data_model(context):
    """Check the data model."""
    assert_true("timestamp" in context.response)
